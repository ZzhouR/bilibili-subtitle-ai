...
(function (global) {
  ...
  function parseTs(ts) {
    ...
    for (const part of p) {
      ...
      if (!isFinite(n)) return NaN; // 非法时间戳交由调用方兜底，避免污染成 NaN 传播
      sec = sec * 60 + n;
    }
    return sec;
  }
  ...
  // 容错：部分接口返回缺失/异常的 to 字段，end<=start 会让播放高亮（t<=end）永远失配，
  // 因此用下一条的 start（或 +5s）补齐，并按 start 升序排序保证二分查找成立。
  function normalizeBody(body) {
    const list = (body || [])
      .map(it => {
        const start = parseTs(it.from ?? it.start ?? 0);
        const end = parseTs(it.to ?? it.end ?? 0);
        return {
      ...
      .filter(it => it.text.length > 0 && isFinite(it.start))
      .sort((a, b) => a.start - b.start);
    for (let i = 0; i < list.length; i++) {
      if (!isFinite(list[i].end) || list[i].end <= list[i].start) {
        const next = list[i + 1];
        list[i].end = next && next.start > list[i].start ? next.start : list[i].start + 5;
      }
    }
    return list;
  }
