...
3. 点「测试连接」验证 →「保存设置」；
4. **使用「📸 截图总结」前**：在设置页「🖼️ 视觉模型」填写视觉 Base URL / API Key / 模型（如通义 `qwen-vl-plus`），用于识别画面中的板书与公式；不配置则截图总结不可用。
...
4. 讲到关键板书时切到「📸 截图总结」→ 点「📷 截图并总结」，识别当前画面后可在下方继续追问；
5. 点「📚 历史」在独立窗口管理所有对话（搜索 / 重命名 / 删除 / 续聊）。
...
- **Markdown/LaTeX 渲染增强**：`$...$`、`$$...$$` 均可渲染；模型漏写 `$` 的裸 LaTeX 命令（`\le`、`\Leftrightarrow`、矩阵等）自动扶正；支持 `---` 分隔线；行内代码不会被误当成公式
...
- [docs/CHANGELOG.md](docs/CHANGELOG.md) —— 变更日志（0.1.0 → 0.10.2）
...
```bash
cd ..
zip -r bilibili-subtitle-ai-v0.10.2.zip bilibili-subtitle-ai -x "*/test/*" "*/docs/screenshots/*"
```
...
**Q：切换分P / 推荐后字幕没变？**
A：确认扩展已刷新（v0.10.2+）。如仍异常，请打开侧边栏观察状态栏文字，并在 Issues 中反馈操作路径。
