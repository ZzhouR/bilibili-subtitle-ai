...
| 入口 | 说明 |
|---|---|
| AI 侧边栏（字幕区） | 字幕列表、轨道切换、当前句显示条、随播放同步高亮滚动、单击勾选 / 双击跳转视频 |
| 工具栏弹窗 | 显示当前视频字幕状态；一键打开 AI 侧边栏 / 设置 |
| AI 侧边栏 | 浏览与勾选字幕行、附加所选字幕/全文为上下文、总结/要点/翻译快捷指令、自定义提问（流式）；发送时自动附带当前字幕全文作为知识库 |
| 对话历史窗口 | 「📚 历史」按钮打开独立窗口：搜索/查看/重命名/删除/载入侧边栏继续对话 |
...
5. 打开任意 B 站视频页，点击扩展图标 →「打开 AI 侧边栏」即可查看字幕（字幕展示全部在侧边栏，无浮动面板）。
...
│   └── subtitle-view.js     # 功能3：播放同步服务（无 UI，广播高亮、响应跳转）
├── lib/
│   ├── wbi.js               # MD5 / wbi 签名 / 字幕解析（纯函数）
│   └── markdown.js          # 零依赖 Markdown 渲染（安全转义 + 链接白名单）
├── sidepanel/               # 功能2：AI 对话侧边栏（panel.html/css/js）
├── history/                 # 功能2-附：对话历史管理独立窗口（history.html/css/js）
├── options/                 # AI 服务设置页
├── popup/                   # 工具栏弹窗
├── assets/                  # 图标（16/48/128）
