...
(() => {
  ...
  const SHOT_SUMMARY_INSTRUCTION =
    ...
    "1. 用标准 Markdown 排版：小标题用 ##，列表用 -，加粗用 **；\n" +
    "2. 一句话说明这一画面在讲什么；\n" +
    "3. 分点整理画面中的关键信息（题目原文 / 步骤 / 结论 / 注意点）；\n" +
    "4. 画面中的公式一律用 $...$ 或 $$...$$ 输出 LaTeX（如 $\\le$、$\\Rightarrow$、$\\begin{vmatrix}...\\end{vmatrix}$），不要输出裸 LaTeX 命令，并简述其含义；\n" +
    "5. 若画面信息不足，明确指出缺什么，不要编造。";
