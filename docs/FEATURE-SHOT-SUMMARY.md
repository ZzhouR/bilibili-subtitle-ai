...
侧边栏「📸 截图总结」→ 点「📷 截图并总结」
  ...
  │     → content SEEK_VIDEO（暂停；当前帧免 seek，否则 seek 后等 seeked/1500ms）→ 返回 video 位置
...
- **当前帧免 seek**：`SEEK_VIDEO` 中若目标与 `currentTime` 差值 ≤0.05s 则跳过 seek —— 写入同值不会触发 `seeked`，否则每次截图都要白等 1500ms 超时。
